...
export default function FileTable({ files, loading, onEdit, onDelete }) {
  return (
            ...
            files.map((row, index) => (
              <tr key={row.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-100"}>
                ...
                <td className="p-3 flex gap-3 justify-center">
                  ...
                  <button onClick={() => onEdit(row)} className="bg-green-500 text-white p-2 rounded-full hover:bg-green-600">
                    <Edit size={16} />
                  </button>
                  <button onClick={() => onDelete(row)} className="bg-red-500 text-white p-2 rounded-full hover:bg-red-600">
                    <Trash2 size={16} />
                  </button>
                </td>
              </tr>
